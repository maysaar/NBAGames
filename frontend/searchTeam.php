<?php
    if (isset($_POST['field_submit'])) {
        ...
        $query = "SELECT DISTINCT team_name FROM teams_info WHERE team_id = :ph_team";
...
<html>
    ...
    <div class="navbar">
            ...
            <div id="searchbg">
               <h1> Search for a team by their ID </h1>
               <br/>
                <form method="post">
                    <label for="id_team">Enter the team's id:</label>
        ...
        if (isset($_POST['field_submit'])) {
            if ($result && $prepared_stmt->rowCount() > 0) { ?>
                <div>
                <h3> The team name for <?php echo $_POST['field_team']; ?> is <?php foreach ($result as $row) { ?>
                         <?php echo $row["team_name"]; ?><?php } ?>. </h3>
                <div id="spacer"></div>
            <?php } else { ?>
                <div>
